...
function ensureAdminInitialized() {
  if (!admin.apps.length) {
    try {
      ...
      // If a duplicate init happens in dev, ignore it; otherwise allow error to bubble later on use
    }
  }
}
...
async function getNovuApiKey(): Promise<string> {
  // Local dev fallback: allow environment variable
