...
async function refreshAccessToken(currentToken: string): Promise<{
  ...
  const endpoint = `https://graph.threads.net/refresh_access_token?grant_type=th_refresh_token&access_token=${currentToken}`;
...
/**
 * Update the THREADS_ACCESS_TOKEN secret with new token
 */
async function updateTokenSecret(newToken: string): Promise<void> {
  const project = process.env.GOOGLE_CLOUD_PROJECT || process.env.GCLOUD_PROJECT || process.env.GCP_PROJECT;
  
  try {
    const parent = `projects/${project}/secrets/THREADS_ACCESS_TOKEN`;
  ...
  if (success) {
    ...
    htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #10b981;">✅ Threads Token Refresh - Success</h2>
        ...
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tr>
            ...
            <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">✅ THREADS_ACCESS_TOKEN updated in Secret Manager</td>
          </tr>
        </table>
        
        <div style="background-color: #eff6ff; border-left: 4px solid #3b82f6; padding: 16px; margin: 16px 0;">
          <p style="margin: 0;"><strong>Next Refresh:</strong> Scheduled for the 7th of next month</p>
        </div>
        
        <p style="color: #6b7280; font-size: 12px; margin-top: 24px;">
          This is an automated report from Kotikreikasta.com Threads token refresh function.<br>
...
- Secret Updated: THREADS_ACCESS_TOKEN updated in Secret Manager

Next Refresh: Scheduled for the 7th of next month

---
This is an automated report from Kotikreikasta.com Threads token refresh function.
Timestamp (UTC): ${timestamp}
    `;
  } else {
    htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #ef4444;">❌ Threads Token Refresh - Failed</h2>
        ...
        <div style="background-color: #fef3c7; border-left: 4px solid #f59e0b; padding: 16px; margin: 16px 0;">
          ...
          <p style="margin: 8px 0 0 0;">
            The Threads access token could not be refreshed automatically. 
            Please manually refresh the token following the instructions in THREADS_SETUP_INSTRUCTIONS.md
          </p>
        </div>
        
        <h3>Manual Refresh Steps</h3>
        <ol style="color: #374151;">
          <li>Go to Graph API Explorer: https://developers.facebook.com/tools/explorer/</li>
          <li>Generate a new short-lived token with threads_basic and threads_content_publish permissions</li>
          <li>Exchange for long-lived token using the command in THREADS_SETUP_INSTRUCTIONS.md</li>
          <li>Update the THREADS_ACCESS_TOKEN secret in Google Secret Manager</li>
        </ol>
...
2. Generate a new short-lived token with threads_basic and threads_content_publish permissions
3. Exchange for long-lived token using the command in THREADS_SETUP_INSTRUCTIONS.md
4. Update the THREADS_ACCESS_TOKEN secret in Google Secret Manager
...
Timestamp (UTC): ${timestamp}
  ...
  const msg = {
    ...
    subject: 'Kotikreikasta.com Threads token refresh report',
    text: textContent,
    html: htmlContent,
  };
...
/**
 * Scheduled function to refresh Threads access token
 * Runs on the 7th of every month at 9:00 AM Helsinki time
 */
export const refreshThreadsToken = functions
  .runWith({ 
    timeoutSeconds: 120,
    memory: '256MB',
  })
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      functions.logger.info('Threads token refresh started');
